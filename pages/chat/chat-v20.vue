...
export default {
  ...
  setup(props) {
    ...
    const isScrollIntoView = ref(true);
    ...
    const handleScrollToUpper = async () => {
      ...
      try {
        ...
        query
            .select('.message-item') // 根据类名获取第一个消息节点
            .fields({
              dataset: true, // 获取 dataset 属性
              id: true, // 如果需要获取节点 ID
              rect: true, // 获取节点布局信息
            }, (res) => {
              if (res && res.dataset?.messageId) {
                const firstMessageId = res.dataset.messageId;
                console.log('当前页面顶部消息 ID:', firstMessageId);
                loadMoreMessages(firstMessageId); // 加载更多消息
              } else {
